...
export default function BookingsManagement() {
  ...
  const updateBookingStatus = async (bookingId: string, newStatus: string) => {
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401 || response.status === 403) {
          console.log(
            "Authentication issue detected while updating status, redirecting to login..."
          );
          window.location.href = "/login?redirect=/guide-dashboard/bookings";
          return;
        }
      ...
      if (data.success) {
        ...
        const studentName = booking?.studentName || "the student";

        // Status-specific messages
        let statusMessage;
        switch (newStatus) {
          case "confirmed":
            statusMessage = `Booking confirmed! ${studentName} has been notified via email.`;
            break;
          case "cancelled":
            statusMessage = `Booking cancelled. ${studentName} has been notified via email.`;
            break;
          case "completed":
            statusMessage = `Booking marked as completed. ${studentName} has been notified via email.`;
            break;
          case "pending":
            statusMessage = `Booking status updated to pending. ${studentName} has been notified via email.`;
            break;
          default:
            statusMessage = `Booking status updated. ${studentName} has been notified via email.`;
        }

        showToast(statusMessage);
        console.log(
          `Successfully updated booking ${bookingId} status to ${newStatus}`
        );
      } else {
        if (data.error === "Invalid or expired token") {
          console.log(
            "Token issue detected during status update, redirecting to login..."
          );
          window.location.href = "/login?redirect=/guide-dashboard/bookings";
          return;
        }
  ...
  const filteredBookings = bookings.filter((booking) => {
    ...
    const matchesSearch =
      ...
      (booking.tourName?.toLowerCase() || "").includes(
        searchTerm.toLowerCase()
      ) ||
      (booking.studentName?.toLowerCase() || "").includes(
        searchTerm.toLowerCase()
      );
    ...
    <div>
      ...
      {error && (
        <div className="flex flex-col items-center justify-center p-8 bg-gray-800 rounded-lg border border-red-800 shadow-lg">
          ...
          >
            <FaSpinner
              className={`mr-2 ${loading ? "animate-spin" : "hidden"}`}
            />
            Try Again
          </button>
        </div>
                              ...
                              <div className="absolute right-0 top-full mt-1 w-32 bg-gray-800 rounded-md shadow-lg border border-gray-700 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-20">
                                {booking.status !== "confirmed" && (
                                  <button
                                    ...
                                    title="Confirm booking and send notification to student"
                                  >
                                ...
                                {booking.status !== "pending" && (
                                  <button
                                    ...
                                    disabled={actionLoading === booking.id}
                                    title="Mark as pending and notify student"
                                  >
                                    {actionLoading === booking.id ? (
                                      <FaSpinner className="animate-spin mr-2" />
                                    ) : (
                                ...
                                {booking.status !== "completed" && (
                                  <button
                                    ...
                                    disabled={actionLoading === booking.id}
                                    title="Mark as completed and notify student"
                                  >
                                    {actionLoading === booking.id ? (
                                      <FaSpinner className="animate-spin mr-2" />
                                    ) : (
                                ...
                                {booking.status !== "cancelled" && (
                                  <button
                                    ...
                                    disabled={actionLoading === booking.id}
                                    title="Cancel booking and notify student"
                                  >
                                    {actionLoading === booking.id ? (
                                      <FaSpinner className="animate-spin mr-2" />
                                    ) : (
                                ...
                                <div className="w-full text-center text-xs text-gray-400 px-2 py-1 border-t border-gray-700 mt-1">
                                  Email notifications will be sent to the
                                  student
                                </div>
                              </div>
                            </div>
            ...
            <div className="bg-gray-800 rounded-lg p-8 border border-gray-700 shadow-lg text-center">
              ...
              <p className="text-gray-400 mb-4">
                {searchTerm || statusFilter !== "all"
                  ...
                  : "You don't have any bookings yet. Once students book your tours, they will appear here."}
              </p>
            </div>
          )}
        </>
